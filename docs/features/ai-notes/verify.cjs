...
(async () => {
  ...
  try {
    for (const width of [1440, 1024, 768, 390, 320]) {
      ...
      check(`${width}: completed features linked`, await page.locator('.feature-entry a').count() === 3 && await page.locator('[data-feature="ai-notes"] a').getAttribute('href') === 'ai-notes.html' && await page.locator('[data-feature="validaciones"] a').getAttribute('href') === 'validaciones.html');
