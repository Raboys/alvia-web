...
| Código | `app/src/components/ui/AuthCodeSheet.tsx`, `coverage-gateway/src/challenges.ts` | «Código de credencial» se relaciona una vez con el nombre de UI «Código de autorización» y la app de la cobertura. No es número de afiliado ni OTP de acceso a Alvia. Tres dígitos en el ejemplo, según la corrección de Pablo sobre el formato habitual; la longitud sigue dependiendo de la cobertura. |
